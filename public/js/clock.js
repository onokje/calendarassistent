...
setCaptions = function() {
  ...
  $('.clockArc.hour').attr('d', describeArc(750, 240, 150, 0, hourArc));
  $('.clockArc.minute').attr('d', describeArc(750, 240, 170, 0, minArc));
  $('.clockDot.hour').attr('d', describeArc(750, 240, 150, hourArc - 3, hourArc));
  $('.clockDot.minute').attr('d', describeArc(750, 240, 170, minArc - 1, minArc));
  dot = $(".clockDot.hour");
  pos = polarToCartesian(750, 240, 150, hourArc);
  dot.attr("cx", pos.x);
  dot.attr("cy", pos.y);
  dot = $(".clockDot.minute");
  pos = polarToCartesian(750, 240, 170, minArc);
  dot.attr("cx", pos.x);
  dot.attr("cy", pos.y);
  return $('#time').text(moment().format('H:mm'));
};

$('#day').text(moment().locale('nl').format('dddd'));
$('#date').text(moment().locale('nl').format('D MMMM'));
